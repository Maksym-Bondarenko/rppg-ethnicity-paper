...
def plot_gender_distribution_horizontal():
    ...
    for i in range(len(datasets)):
        ax.text(female[i] / 2, i, f"{female_pct[i]}%", va='center', ha='center', fontsize=9, color="white", weight="bold")
        ax.text(female[i] + male[i] / 2, i, f"{male_pct[i]}%", va='center', ha='center', fontsize=9, color="white", weight="bold")

    ax.set_yticks(y)
    ax.set_yticklabels(datasets, fontsize=12)
    ax.set_title("Gender Distribution Across Datasets", fontsize=18, fontweight="bold")
...
def plot_ethnicity_boxplot_with_pvalues():
    ...
    y_max = df["Proportion (%)"].max() + 5
    y_step = 5
    for (group1, group2), p_value in p_values.items():
        x1, x2 = list(df["Ethnicity"].unique()).index(group1), list(df["Ethnicity"].unique()).index(group2)
        y = y_max
